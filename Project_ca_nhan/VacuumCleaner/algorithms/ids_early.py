    ...
    def depth_limited_search(self, start_state, limit, step, logs):
        ...
        while stack:
            ...
            if step % 100 == 0 or current_depth == limit:
                logs.append(f"IDS Early Duyệt Node #{step} | Vị trí: ({state[0]},{state[1]}) | Tầng: {current_depth}/{limit}")
            ...
            for action in self.actions(state[0], state[1], state[2]):
                ...
                if new_node_key not in path_states:
                    if self.is_goal(new_state[2]):
                        logs.append(f"-> [Early] Tìm thấy đích thành công tại bước thứ #{step + 1}!")
                        return "FOUND", path + [action], step, logs
